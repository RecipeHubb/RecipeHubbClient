...
// import PocketBase from "../../../service/PocketBase";
...
export default {
  ...
  methods: {
    createRecipe: async function() {
      ...
      // const record = await PocketBase.records.create('recipes', formData)
      const res = await RecipeService.createRecipe({
        name: this.name,
        ingredients: this.ingredients,
        instructions: this.instructions,
        // recipeImage: record.image,
        recipeImage: this.previewImage,
    ...
    previewImage: function(event) {
      ...
      if (input.files && input.files[0]) {
        ...
        this.recipeImage = input.files[0];
        reader.onload = e => {
